...
// Hay que escuchar la funcion de polymer de cuando un componente esta listo (mirar webcomponents)
      document.addEventListener('WebComponentsReady', function() {
        ...
        //creo un hash para no tener problemas con el tamano que acepta mixpanel en sus eventos (255 es el limite de caracteres)
        var hash = sha1.create();
        var hash_input = publish_text;
        //hasheo con comillas simples, porque Python lo hace con comillas simples
            ...
            if(element.data[i].current_date === last_date && element.data[i].current_time === last_time){
              ...
              if(element.data[i].dt.getDate()===dd){
                for (var i=0; i<publish_text.length;i++){
                 //comparo datos del componente con los de la api(python) para asegurarme que se trata del mismo post
                  //si es el mismo, se mandan los datos a mixpanel
